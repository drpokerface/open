...
# v10.6: a model call that never delivered a verdict because the WORLD is throttled (quota, 429, rate limit,
# billing) is not evidence about the artifact. The kit halts verify.py with a distinct signal so the loop can
# tell "the verifier could not run" apart from "the work is wrong" - the same distinction v10.3 gave the audit.
WALL_MARKS = ("resource_exhausted", "resource exhausted", "429", "quota", "rate limit", "rate-limit", "ratelimit",
              "rate_limit", "too many requests", "billing", "spending cap", "overloaded", "unavailable", "503")
def _is_wall(err):
    s = str(err).lower()
    return any(m in s for m in WALL_MARKS)
...
class Kit:
    ...
    def _wall(self, cid, claim, err):
        # v10.6: a resource wall during a model call - stop the whole verifier with a distinct exit code and marker,
        # so the loop treats it as "verification could not run", never as a FAIL of the artifact
        print(cid + ": UNAVAILABLE - " + claim + " - verification could not run: " + str(err)[:160])
        print("VERIFICATION-UNAVAILABLE: a model call in verify.py hit a resource wall (quota / rate limit / 429) - "
              "not a work failure; the gate abstains and the task retries later")
        sys.exit(3)
